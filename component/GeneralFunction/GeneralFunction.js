...
export const getNamaBulan = (urutanBulan) => {
    ...
    if (urutanBulan >= 1 && urutanBulan <= 12) {
        return namaBulanArray[urutanBulan - 1];
    } else {
...
export const currentTime = new Date().toLocaleTimeString('id-ID', {
    hour: '2-digit',
    minute: '2-digit',
    timeZoneName: 'short',
});
...
export const getFaktur = async (key, length) => {
    let requestBody = {
        Kode: key,
        Len: length
    };
    const vaData = await postData('/api/get_faktur', requestBody);
    const data = vaData.data;
    return data;
};
