...
import { convertToISODate, formatAndSetDate, formatDate, formatDatePdf, formatDateTable, getEmail, getUserName, showError, showSuccess } from '../../../component/GeneralFunction/GeneralFunction';
...
export default function MutasiGudang() {
    ...
    const [jenisMutasiOption, setJenisMutasiOption] = useState('Semua');
    ...
    const exportPDF = async (dataAdjust) => {
        try {
            ...
            const tableData = mutasiGudangPDF.map((item) => [item.Faktur, formatDatePdf(item.Tgl), item.KetGudangKirim, item.KetGudangTerima, item.FullName]);
