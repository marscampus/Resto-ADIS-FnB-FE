...
export default function MasterPelunasanHutang(){
    ...
    const [pelunasanHutangDialog, setPelunasanHutangDialog] = useState(false);
    ...
    const hideDialog = () => {
        setSubmitted(false);
        setPelunasanHutangDialog(false);
    };

    // -------------------------------------------------------------------------------------------------------------------- Dialog
    const router = useRouter();
    const openNew = () => {
        setPelunasanHutang(emptypelunasanhutang);
        setSubmitted(false);
        router.push('/pembelian/pelunasan-hutang/add');
        setStatusAction('store');
    };

    // -------------------------------------------------------------------------------------------------------------------- Func
    const savePelunasanHutang = async (e)=>{
        e.preventDefault();
    }
    ...
    const pelunasanHutangDialogFooter = (
        <>
            <Button label="Cancel" icon="pi pi-times" className="p-button-text" onClick={hideDialog} />
            <Button label="Save" icon="pi pi-check" className="p-button-text" onClick={savePelunasanHutang} />
        </>
    );
    ...
    return (
        <div className="grid crud-demo">
            <div className="col-12">
                ...
                <div className="card">
                    <h4>Menu Pelunasan Hutang</h4><hr/>
                    <Toast ref={toast} />
                    <Toolbar className="mb-4" left={leftToolbarTemplate} right={rightToolbarTemplate}></Toolbar>
                    <DataTable
                        // value={pelunasanHutang}
                        lazy
                        // dataKey="KODE"
                        // paginator
                        rows={10}
                    ...
                    {/* Dialog PelunasanHutang  */}
                    <Dialog visible={pelunasanHutangDialog} style={{ width: '75%' }} header="Tambah PelunasanHutang " modal className="p-fluid" footer={pelunasanHutangDialogFooter} onHide={hideDialog}>
                        <div className="formgrid grid">
                            <div className="field col-12 mb-2 lg:col-6">
                                <label htmlFor="kode">Kode</label>
                                <div className="p-inputgroup">
                                    <InputText disabled id="kode" value={pelunasanHutang.KODE} onChange={(e) => onInputChange(e, 'KODE')} className={classNames({ 'p-invalid': submitted && !pelunasanHutang.KODE })} />
                                </div>
                                {submitted && !pelunasanHutang.KODE && <small className="p-invalid">Kode is required.</small>}
                            </div>
                            <div className="field col-12 mb-2 lg:col-6">
                                <label htmlFor="keterangan">Keterangan</label>
                                <InputText id="keterangan" value={pelunasanHutang.Keterangan} onChange={(e) => onInputChange(e, 'Keterangan')} required autoFocus className={classNames({ 'p-invalid': submitted && !pelunasanHutang.Keterangan })} />
