...
export default function addProduk() {
    ...
    const fetchDataForEdit = async () => {
        ...
        try {
            ...
            if (json.data.STATUS_STOCK === "1") {
                setStokAwal(true);
            }
            setProduk(json.data);
            setGolonganKet(json.data.KETGOLONGAN);
            setRakKet(json.data.KETRAK);
            setGudangKet(json.data.KETGUDANG);
            setSupplierNama(json.data.NAMASUPPLIER);
            setVariant(json.data.VARIANT);

            const initialSatuanList = [
                { id: 1, value: json.data.SATUAN || '', keterangan: json.data.KETSATUAN1 || '' },
                ...(json.data.SATUAN2 || json.data.KETSATUAN2 ? [{ id: 2, value: json.data.SATUAN2, keterangan: json.data.KETSATUAN2 }] : []),
                ...(json.data.SATUAN3 || json.data.KETSATUAN3 ? [{ id: 3, value: json.data.SATUAN3, keterangan: json.data.KETSATUAN3 }] : [])
            ];
            setSatuanList(initialSatuanList);

            const initialHargaBeliList = [];
            if (json.data.HB > 0) initialHargaBeliList.push({ id: 1, value: json.data.HB, satuan: '' });
            if (json.data.HB2 > 0) initialHargaBeliList.push({ id: 2, value: json.data.HB2, satuan: '' });
            if (json.data.HB3 > 0) initialHargaBeliList.push({ id: 3, value: json.data.HB3, satuan: '' });
            setHargaBeliList(initialHargaBeliList);

            const initialHargaJualList = [];
            if (json.data.HJ > 0) initialHargaJualList.push({ id: 1, value: json.data.HJ, satuan: '' });
            if (json.data.HJ2 > 0) initialHargaJualList.push({ id: 2, value: json.data.HJ2, satuan: '' });
            if (json.data.HJ3 > 0) initialHargaJualList.push({ id: 3, value: json.data.HJ3, satuan: '' });
